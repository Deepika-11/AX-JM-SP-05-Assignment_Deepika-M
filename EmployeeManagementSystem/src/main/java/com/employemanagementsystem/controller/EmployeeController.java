...
public class EmployeeController {
    ...
    public ResponseEntity<?> saveEmployeeDetails(@RequestBody Employee employee) {
        if (!employeeServiceImpl.saveEmployee(employee)) {
            return new InvalidEmployeeIdException("Employee is already found / The given department id is not found").handleEmployeeException();
        }
    ...
    public ResponseEntity<?> getEmployeeById(@PathVariable("id") int id) {
        ...
        if (employee.isEmpty()) {
            return new InvalidEmployeeIdException("Employee Id not found").handleEmployeeException();
        }
    ...
    public ResponseEntity<?> getEmployeeBySalary(@PathVariable double salary){
        ...
        if (employee.isEmpty()) {
            return new InvalidEmployeeIdException("The given salary is not found").handleEmployeeException();
        }
        return new ResponseEntity<>(employee, HttpStatus.FOUND);
    }
    ...
    public ResponseEntity<?> getEmployeeByDeptName(@PathVariable String name){
        ...
        if (employee.isEmpty()) {
            return new InvalidEmployeeIdException("The given department name is not found").handleEmployeeException();
        }
        return new ResponseEntity<>(employee, HttpStatus.FOUND);
    }
